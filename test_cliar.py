...
def test_positional_args(capfd):
    ...
    run(f'python testmath.py add {x} {y}', shell=True)
    assert int(capfd.readouterr().out) == x + y

def test_optional_args(capfd):
    x, power = 12, 3

    run(f'python testmath.py power {x} --power {power}', shell=True)
    assert int(capfd.readouterr().out) == x ** power

    run(f'python testmath.py power {x} -p {power}', shell=True)
    assert int(capfd.readouterr().out) == x ** power

    run(f'python testmath.py power {x}', shell=True)
...
def test_nargs(capfd):
    ...
    run(f'python testmath.py summ {" ".join(str(number) for number in numbers)}', shell=True)
    assert int(capfd.readouterr().out) == sum(numbers)

    numbers = 1, 2, 42.2, 101.1

    run(
        f'python testmath.py avg --numbers {" ".join(str(number) for number in numbers)}',
        shell=True
    )
    assert float(capfd.readouterr().out) == sum(numbers)/len(numbers)

    run(f'python testmath.py avg -n {" ".join(str(number) for number in numbers)}', shell=True)
    assert float(capfd.readouterr().out) == sum(numbers)/len(numbers)
...
def test_missing_args(capfd):
    ...
    run(f'python testmath.py add {x}', shell=True)
    assert capfd.readouterr().err.splitlines()[-1].endswith(error.format(args='y'))

    run(f'python testmath.py add', shell=True)
...
def test_redundant_args(capfd):
    ...
    run(f'python testmath.py add {x} {y} {z}', shell=True)
    ...
    run(f'python testmath.py add {x} {y} {z} {q}', shell=True)
...
def test_open(capfd):
    ...
    run('python testmath.py sumfile numbers.txt', shell=True)
...
def test_aliases(capfd):
    ...
    run(f'python testmath.py sum {x} {y}', shell=True)
    assert int(capfd.readouterr().out) == x + y

    run(f'python testmath.py plus {x} {y}', shell=True)
    assert int(capfd.readouterr().out) == x + y
...
def test_help(capfd):
    ...
    run('python testmath.py add -h', shell=True)
    help_message = capfd.readouterr().out
    assert 'Add two numbers.' in help_message
    assert 'First operand' in help_message
    assert 'Second operand' in help_message
...
def test_ignore(capfd):
    ...
    run(f'python testmath.py calculate-factorial {n}', shell=True)
...
def test_pseudonym(capfd):
    ...
    run(f'python testmath.py fac 6', shell=True)
...
def test_arg_map(capfd):
    ...
    run(f'python testmath.py log {x} --to {base}', shell=True)
    assert float(capfd.readouterr().out) == log(x, base)

    run(f'python testmath.py log {x} -t {base}', shell=True)
    assert float(capfd.readouterr().out) == log(x, base)


def test_metavars(capfd):
    run('python testmath.py log --help', shell=True)
